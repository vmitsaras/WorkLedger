...
- [ ] `WL-1508K` Select a source-reviewed stable provider candidate with schema enforcement under think:false, then qualify exact installation/model/isolation using cold-start health and 18/18 synthetic challenges under spec 0003. J complete; repair output-writability/persistence preparation from report 175 before inference. No candidate selected or model execution authorized by J verification. Fresh B then C required afterward.
