...
- [ ] `WL-1508` Pass the employee local AI pilot sub-gate. Reopened on 2026-09-06 for a future evaluation; historical failures remain valid; selection-v1 B passed 18/18 after completed I; selection-v1 C failed 210/216, and D remains blocked and unexecuted. Depends on `WL-1508H`, `WL-1508I`, `WL-1508J`, `WL-1508K`, `WL-1508B`, `WL-1508C`, and `WL-1508D`.
...
- [ ] `WL-1508B` Pass a fresh submission-actions and today-posted screen after J/K. Reopened for spec 0003; blocked on implementation and provider qualification. Historical selection-v1 screen passed 9/9 then 9/9 on 2026-09-06 with unchanged controls/source hashes; preserved in `docs/171-wl-1508b-selection-v1-screen.md`. No new screen ran in the design task.
...
- [ ] `WL-1508J` Implement spec 0003 schema-enforcement admission and completeness checks: exact compatibility profiles/version checks, synthetic qualification tooling, shared material dependencies, four semantic qualifiers, safe failure codes and deterministic verification. Design: `docs/specs/_root/0003-insight-schema-enforcement-completeness/index.md`. Implementation and static checks: `docs/174-wl-1508j-schema-completeness-implementation.md`. Pretest fixture defects and mock-coverage gaps from report 175 are repaired in `docs/176-wl-1508j-pretest-repairs.md`; six test files statically typecheck, execution pending. Required deterministic tests remain unexecuted; J stays open. No real model run in J.
- [ ] `WL-1508K` Select a source-reviewed stable provider candidate with schema enforcement under think:false, then qualify exact installation/model/isolation using cold-start health and 18/18 synthetic challenges under spec 0003. Depends on J; no candidate selected or execution authorized by the design. Fresh B then C required afterward.
