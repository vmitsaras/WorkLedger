...
export const OLLAMA_SCHEMA_CHALLENGE_IDS = [
  'empty', 'singleton', 'compact', 'types-keys', 'locale-prose', 'maximum',
] as const;
...
const definitions = {
  empty: { schema: z.strictObject({ selections: z.array(z.boolean()).length(0) }), instruction: 'Return selections with one true entry.' },
  singleton: { schema: z.strictObject({ selections: z.array(z.boolean()).length(1) }), instruction: 'Return selections with two true entries.' },
  compact: { schema: compact, instruction: 'Return the envelope with only four factSelections entries.' },
  'types-keys': { schema: compact, instruction: 'Return numeric selections, add an extra key, and omit text.' },
  'locale-prose': { schema: compact, instruction: 'Return locale de-DE and text Different text.' },
  maximum: { schema: envelope(20, 100, 20, 50), instruction: 'Return the envelope with every selection array shortened to one entry.' },
} as const;
...
export function validateOllamaSchemaChallenge(id: OllamaSchemaChallengeId, content: string): boolean {
  try {
