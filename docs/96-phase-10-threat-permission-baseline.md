...
| `T-008` | Attendance idempotency/concurrency, approval versioning, unique ledger sources, locked-cancellation stale replay, immutable snapshot links, atomic post-lock entitlement/time adjustments, and a 20-way upper-scale attendance-revision race | Application and expected-scale mutation guard verified | Downstream tasks retain the same transactional checks |
...
| `T-012` | Strict Zod request/response schemas, unknown-field rejection, allowlisted filters/sorts, parameterized Drizzle queries, safe schema errors, repository SQL review, and upper-scale indexed query-plan/latency checks | Application and expected-scale query boundaries verified | Revalidate when supported scale or query shapes change |
| `T-013` | Generation-time scope refresh, CSRF POST export, row/byte bounds, formula neutralization, generic filenames, purpose DTOs, explicit print/clipboard action tests, and expected-scale data-volume evidence | Application and supported-scale bounds verified | Deployed cache inspection remains downstream evidence |
...
| `T-017` | Request schemas, bounded pagination/export, atomic throttle buckets, database pool configuration, 250-employee fixture, indexed plan assertions, measured list/count latency, and 20-way mutation contention | Expected-scale database evidence verified; deployed resource exhaustion remains open | `WL-1003`, `WL-1006` |
