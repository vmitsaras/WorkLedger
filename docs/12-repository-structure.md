...
│   │   └── test/
│   │
│   └── site/                      # Added only when the portfolio draft is scheduled
│
├── packages/
│   ├── domain/
│   │   ├── src/
...
- `apps/site`, when added under the unnumbered portfolio draft, may import UI only unless a later ADR justifies another edge.
