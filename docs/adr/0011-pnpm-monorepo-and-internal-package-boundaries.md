...
- Workspace projects are `apps/web`, `apps/api`, `packages/domain`, `packages/contracts`, `packages/database`, `packages/ui`, `packages/config`, and `packages/test-utils`. `apps/site` is added only when the unnumbered portfolio draft is scheduled.
