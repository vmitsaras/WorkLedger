...
| WL-1508B | Run the qualified candidate through the known-failure regression screen | WL-1508A | `qwen2.5-coder:14b` stopped at 0/9 `submission-actions`: every locale/repetition returned content-free `PROVIDER_INVALID_OUTPUT` / `INVALID_RESPONSE` / `TOOL_REQUIRED`; `today-posted` was not run; no evaluation contract changed | In progress — failed candidate |
