...
| WL-1508B | Run the qualified candidate through the known-failure regression screen | WL-1508A, WL-1508E, WL-1508F | `qwen2.5-coder:14b` stopped at 0/9 invalid tool-selection output; exact `qwen3-coder:30b` then stopped at 0/9 provider timeouts; resume only after WL-1508F and only for the exact qualified `qwen2.5-coder:14b` digest | Blocked by WL-1508F |
...
| WL-1508F | Make Employee Insight registry execution server-owned and constrain the single provider response with the existing structured-output contract | WL-1508A, WL-1508E | `docs/specs/_root/0001-server-owned-insight-orchestration.md`; implementation and deterministic evidence complete; exact registry reauthorization before provider context; one tool-free schema-constrained generation; unchanged runtime validators, provider controls, endpoint, UI, and data model; no real model case; closure pending review | In progress |
