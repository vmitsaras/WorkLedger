...
| WL-1505 | Implement a reusable read-only Insight tool registry with independent authorization and active-workspace scope | WL-1504 | Narrow purpose-specific tools; deny-by-default policy metadata; current employee/manager scope; combined-role isolation; no generic query/SQL capability | Not started |
| WL-1506 | Implement an AI-provider abstraction and optional local Ollama adapter with configuration, capability, and health checks | WL-1505 | Disabled by default; validated private endpoint/model configuration; no silent external egress; timeouts/cancellation; health failure leaves deterministic Insights usable | Not started |
| WL-1507 | Implement employee-only Ask My Ledger interpretation, tool orchestration, structured output, cancellation, and source attribution | WL-1506 | Session-only context; self-scoped tools only; conversational follow-ups preserve visible scope; every answer maps to validated facts/actions and cites source records | Not started |
| WL-1508 | Add the employee golden-question evaluation set, bounded traces, privacy/security tests, model-failure handling, and accessibility verification, then execute the local AI pilot sub-gate | WL-1507 | 20–30 curated questions; 100% numeric, scope, permission, source, and native-action correctness; zero unsupported claims or sensitive leakage; prompt-injection and degraded-provider evidence | Not started |
