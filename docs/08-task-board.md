...
| WL-1508 | Add the employee golden-question evaluation set, bounded traces, privacy/security tests, model-failure handling, and accessibility verification, then execute the local AI pilot sub-gate | WL-1507, WL-1508A–WL-1508D | `docs/159-wl-1508-employee-local-ai-pilot-evaluation.md`; evaluation infrastructure and non-model evidence complete; parent closes only after one exact replacement digest records 216/216 and every child task is complete | In progress |
| WL-1508A | Qualify one replacement private model name and exact digest | WL-1507 | Candidate becomes ready within the accepted 120-second startup boundary; exact name/digest and required chat, structured-output, and tool capabilities pass; origin, proxy, redirect, egress, retention, and disabled-by-default controls remain unchanged | Ready |
| WL-1508B | Run the qualified candidate through the known-failure regression screen | WL-1508A | All `submission-actions` and `today-posted` cases pass in all three locales and all three repetitions; content-free smoke artifact retained; no prompt, schema, validator, threshold, or native-fallback change | Blocked by WL-1508A |
