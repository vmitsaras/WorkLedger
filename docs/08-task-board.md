...
| WL-1508 | Add the employee golden-question evaluation set, bounded traces, privacy/security tests, model-failure handling, and accessibility verification, then execute the local AI pilot sub-gate | WL-1507, WL-1508A–WL-1508E | `docs/159-wl-1508-employee-local-ai-pilot-evaluation.md`; evaluation infrastructure and non-model evidence complete; parent closes only after one exact replacement digest records 216/216 and every child task is complete | In progress |
...
| WL-1508B | Run the qualified candidate through the known-failure regression screen | WL-1508A, WL-1508E | `qwen2.5-coder:14b` stopped at 0/9 `submission-actions`: every locale/repetition returned content-free `PROVIDER_INVALID_OUTPUT` / `INVALID_RESPONSE` / `TOOL_REQUIRED`; after WL-1508E, a replacement must pass both semantic groups without an evaluation-contract change | Blocked by WL-1508E |
| WL-1508E | Qualify the remaining installed `qwen3-coder:30b` digest before resuming the regression screen | WL-1508A | Exact explicit tag and digest; empty pre-probe loaded-model state; ready within 120 seconds with chat, structured-output, and tool capabilities; synthetic probe only; no pull, employee case, prompt/schema/validator change, or provider-boundary change | Ready |
| WL-1508C | Execute the complete strict replacement-model evaluation | WL-1508B | One uninterrupted 24-question × 3-locale × 3-repetition run records 216/216 for the exact qualified digest; artifact remains content free; any failure stops the sequence without gate closure | Blocked by WL-1508B |
| WL-1508D | Reconfirm evidence and close the employee local AI pilot sub-gate | WL-1508C | Applicable quality gates and unchanged privacy, security, degraded-provider, trace, and accessibility evidence pass; evaluation and project-memory documents name the exact passing digest; WL-1508 closes while provider mode remains disabled by default | Blocked by WL-1508C |
| WL-1509 | Implement deterministic manager action-summary and team-coverage insights | WL-1504, WL-1508 | Current-direct-report scope, neutral `UNAVAILABLE`, explicit configured threshold or factual counts only, source actions, and timezone/date fixtures | Not started |
| WL-1510 | Extend AI interpretation to manager Insights with neutral availability semantics and no employment-decision recommendations | WL-1509 | Active Manager workspace required; no self-scope blending, sickness subtype, staffing judgment, ranking, approval recommendation, or write action | Not started |
| WL-1511 | Implement validated natural-language report/filter generation and native Open report, Apply filters, and Save view actions | WL-1510 | Model emits an allowlisted report specification; WorkLedger authorizes and executes it; saved views persist only normal non-sensitive report/filter state | Not started |
| WL-1512 | Define purpose-specific HR aggregate contracts, privacy thresholds, cohort suppression, and model-context minimization | WL-1508, WL-1511 | Accepted ADR/data-flow review; suppression occurs before model context; no free text, diagnosis, note, attachment, or row-level sickness data reaches the model | Not started |
| WL-1513 | Add HR aggregate Insights only after the dedicated privacy and authorization gate passes | WL-1512 | Purpose-specific metrics distinguish employees, cases, days, and minutes; cohort/scope tests prevent inference and leakage; no decision or prediction output | Not started |
| WL-1514 | Add isolated System Insights using technical diagnostics only and no employee or HR data | WL-1508 | System role receives allowlisted health, migration, backup, mail, session-policy, model-health, and version facts only; public diagnostic minimization remains intact | Not started |
| WL-1515 | Evaluate an optional MCP adapter over explicitly allowlisted Insight tools | WL-1508, WL-1511, WL-1513, WL-1514 | Separate ADR and threat review; per-tool exposure metadata; HR aggregates disabled by default; authentication, authorization, audit, rate, and transport boundaries verified | Not started |
| WL-1516 | Execute multilingual, accessibility, security, privacy, usability, model-failure, upgrade, and Phase 15 release gates | WL-1509–WL-1515 | Signed Phase 15 checklist; deterministic no-provider and Ollama-enabled paths; no open P0/P1 defect; synchronized documentation; ten manifests at `0.16.0` | Not started |
