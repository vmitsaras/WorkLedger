...
See spec 0005 and reports 192–194. Fresh cold health and 18/18 schema qualification passed; report 195 B failed 3/9 on fact/action omissions; diagnosis precedes another attempt. Deployment remains disabled.
...
report 178 records verified portable provenance, active outbound blocking, successful cold health
and 18/18 synthetic qualification. The candidate was stopped afterward; fresh B/C remain required
and deployment remains disabled. Material dependency omissions
...
| Account and security metadata | Authentication/system administration | PostgreSQL; purpose-specific self/technical DTO | Security/technical; personal | Account state, locale, session list, revocation, rate limiting, incident review | Account/security metadata | Self locale/session control; eligible administrator action; deactivation | Technical role learning HR facts or storing excessive IP/device history | Separate DTOs; allowlisted locale; opaque session IDs; never return token; minimize IP/UA; technical authorization |
...
| Low-sensitivity preferences | Signed-out language (`workledger.locale`), theme/density/agenda preference | Combining with account, employee, organization, request, search, or route history; allowing a device value to override an authenticated account | Moderate | Only documented allowlisted display preferences may persist locally under bounded keys; discard invalid values, tolerate storage failure, and keep authenticated account state authoritative |
...
- Only documented low-sensitivity display preferences may persist locally. The signed-out language
  key is exactly `workledger.locale`; its value is restricted to the production locale allowlist,
  is organization/account-independent, and is removed when invalid. Storage failure is non-fatal,
  and no route/search/person/request history accompanies it. An authenticated account's PostgreSQL
  locale always overrides the device value.
...
- The self-locale mutation accepts only the exact locale allowlist and addresses the current account
  exclusively; it accepts no caller-supplied account identifier. It uses the same origin, session,
  CSRF, strict validation, response serialization, and `private, no-store` controls. A locale change
  does not create a domain/security audit event because it changes neither authorization nor
  operational history.
